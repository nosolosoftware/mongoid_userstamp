...
describe 'Mongoid::Userstamp' do
  ...
  subject { Book.new(:name => 'Ethyl the Aardvark Goes Quantity Surveying') }
  ...
  context 'when saved without a user' do
    before { subject.save! }
  ...
  context 'when saved by a user' do
    let(:user_1){ User.create!(:name => 'Charles Dikkens') }
    before { User.current = user_1; subject.save! }
    ...
    context 'when updated by a user' do
      ...
      before { User.current = user_2; subject.save! }
      it { subject.created_by.should == user_1.id }
      it { subject.creator.should    == user_1 }
